...
async function caricaCatalogo(catalogoDemo) {
  try {
    const PAGE = 1000;
    let tutti = [], offset = 0;
    const cols = "cod,nome,descrizione,desc_prev,categoria,marchio,tipologia,um,listino,sconto,netto,tipo_prezzo,note,img";
    while (true) {
...
export default function App(){
  ...
  const [roleInput, setRoleInput] = useState("commerciale");
  ...
  const r = role ? RUOLI[role] : null;

  function login(){ setRole(roleInput); setArea("home"); setMsgs([]); }
  function logout(){ setRole(null); setArea("home"); setMsgs([]); setCart([]); }
  ...
  function aiReply(q, cat){
    ...
    return `${CATALOG.length} articoli demo indicizzati. Chiedi per categoria, marchio o naviga dal menu.`;
  }

  if(!role) return <Login roleInput={roleInput} setRoleInput={setRoleInput} onLogin={login}/>;
    ...
    <div style={S.app}>
      ...
      <div style={S.main}>
        ...
        <div style={S.content}>
          ...
          {area==="preventivi" && <Preventivi cart={cart} setCart={setCart} preventivi={preventivi} setPreventivi={setPreventivi} setOrdini={setOrdini} setArea={setArea} ruolo={role} catalog={catalog} cliente={cliente} setCliente={setCliente}/>}
          {area==="ordini" && <Ordini ordini={ordini} setOrdini={setOrdini} cliente={cliente} setCliente={setCliente}/>}
          {area==="interventi" && <Interventi cliente={cliente} setCliente={setCliente}/>}
          ...
          {area==="admin" && <PannelloAdmin setCatalog={setCatalog} ruolo={role}/>}
        </div>
...
// ─── LOGIN ──────────────────────────────────────────────────────────────────────
function Login({roleInput,setRoleInput,onLogin}){
  ...
  const L = dark ? {
    bg: C.inkDeep,
    card: C.surface,
    cardBorder: C.surfaceRaised,
    labelColor: C.steel,
    subColor: C.steelLight,
    selectBg: C.inkDeep,
    selectColor: "#fff",
    selectBorder: C.surfaceRaised,
    footerColor: "#4A5680",
  } : {
    bg: "#F4F2EC",
    card: "#fff",
    cardBorder: C.paperLine,
    labelColor: "#9AA3AB",
    subColor: "#6B7280",
    selectBg: "#fff",
    selectColor: C.charcoal,
    selectBorder: C.paperLine,
    footerColor: "#9AA3AB",
  };
  ...
  return (
    <div style={{minHeight:"100dvh",background:L.bg,display:"flex",alignItems:"center",justifyContent:"center",padding:20,fontFamily:F_BODY,transition:"background 0.3s"}}>
      ...
      <div style={{maxWidth:360,width:"100%"}}>
        {/* Logo: Telos a sinistra, Telos Tech a destra — allineati ai bordi della card */}
        ...
        <div style={{background:L.card,border:`1px solid ${L.cardBorder}`,borderRadius:10,padding:"24px 22px",transition:"background 0.3s,border-color 0.3s"}}>
          <div style={{fontFamily:F_MONO,fontSize:10.5,fontWeight:600,color:L.labelColor,textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:6}}>Accesso demo</div>
          <div style={{fontSize:11.5,color:L.subColor,marginBottom:14,lineHeight:1.5}}>Seleziona il profilo da provare</div>
          <select style={{...S.sel,width:"100%",padding:"11px 12px",fontSize:13.5,background:L.selectBg,color:L.selectColor,border:`1px solid ${L.selectBorder}`,marginBottom:14}} value={roleInput} onChange={e=>setRoleInput(e.target.value)}>
            <option value="commerciale">Commerciale</option>
            <option value="tecnico">Tecnico</option>
            <option value="responsabile">Responsabile</option>
            <option value="admin">Amministratore</option>
          </select>
          <button onClick={onLogin} style={{...S.btnAccent,width:"100%",padding:"13px",fontSize:14}}>Accedi al sistema</button>
        </div>
        <div style={{fontSize:10.5,color:L.footerColor,textAlign:"center",marginTop:18,lineHeight:1.6,fontFamily:F_MONO}}>
          AUTENTICAZIONE REALE VIA EMAIL + MFA<br/>NEI MODULI DI PRODUZIONE
        </div>
      </div>
    </div>
  );
...
function AIChat({msgs,msgInput,setMsgInput,sendMsg}){
  return (
    <div style={{display:"flex",flexDirection:"column",height:"calc(100dvh - 54px - 100px)"}}>
...
function Prodotti({cart,setCart,catalog:catProp,catalogLoading}){
  ...
  const searchIndex = useMemo(()=>buildSearchIndex(CATS),[CATS]);
    ...
    <div>
      ...
      {aiSearching && (
        <div style={{display:"flex",alignItems:"center",gap:6,fontSize:11.5,color:"#8A929A",marginBottom:8}}>
          <span style={{width:6,height:6,borderRadius:"50%",background:C.ink,animation:"none"}}/>
      ...
      <div style={{...S.eyebrow,display:"flex",justifyContent:"space-between"}}><span>Catalogo</span><span className="tnum">{filtered.length} articoli</span></div>
      {filtered.length===0 && q.trim() && !aiSearching && (
...
function Preventivi({cart,setCart,preventivi,setPreventivi,setOrdini,setArea,ruolo,catalog,cliente,setCliente}){
  ...
  if((view==="dettaglio"||view==="dettagli-edit") && selezionato){
    ...
    return (
      <div>
        ...
        {editable ? (
          <input value={selezionato.cliente} onChange={e=>aggiorna(selezionato.id,{cliente:e.target.value})} placeholder="Nome cliente" style={{...S.inp,marginBottom:14,fontSize:16,fontWeight:600}}/>
        ) : (
  ...
  return (
    <div>
      <div style={{marginBottom:14}}>
        <SelezioneCliente onSeleziona={setCliente} clienteSelezionato={cliente}/>
      </div>
...
const sbAdmin = (method, path, body) => fetch(`${SB_URL}/rest/v1/${path}`, {
  method,
  headers: {
...
function PannelloAdmin({ setCatalog, ruolo }) {
  ...
  useEffect(() => {
    sbAdmin("GET", "prodotti?select=count&attivo=eq.true")
      .then(r => r.json())
      .then(d => setConteggio(d?.[0]?.count ?? "—"))
      .catch(() => setConteggio("—"));
  }, [importando]);
    ...
    try {
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        for (let tentativo = 0; tentativo < 3; tentativo++) {
          const r = await sbAdmin("POST", "prodotti", chunks[i]);
          if (r.ok) {
        ...
        if (!ok) {
          ...
          for (const prod of chunks[i]) {
            const r2 = await sbAdmin("POST", "prodotti", [prod]);
            if (r2.ok) { done++; }
            else { errori++; }
  ...
  async function svuotaEThenImport() {
    ...
    const r = await sbAdmin("DELETE", "prodotti?attivo=eq.true");
    if (r.ok || r.status === 204) {
      addLog("Catalogo svuotato. Avvio import…", "ok");
      await avviaImport();
    } else {
      addLog(`Errore svuotamento: ${r.status}`, "errore");
      setImportando(false);
    }
  }
  ...
  async function esportaCatalogo() {
    ...
    try {
      ...
      let tutti = [], offset = 0;
      while (true) {
        const r = await sbAdmin("GET", `prodotti?select=cod,nome,descrizione,desc_prev,categoria,marchio,tipologia,um,listino,sconto,netto,tipo_prezzo,note&order=categoria,marchio,cod&limit=${PAGE}&offset=${offset}`);
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",borderBottom:`1px solid ${C.paperLine}`,marginBottom:18}}>
        {[["import","⬆ Importa"],["export","⬇ Esporta"]].map(([id,lbl])=>(
